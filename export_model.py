...
class ModelExporter(object):
    ...
    def export_model(self, model_dir, global_step_val, last_checkpoint):
        with self.graph.as_default():
            with tf.Session() as sess:
                ...
                self.saver.restore(sess, last_checkpoint)
                signature = signature_def_utils.build_signature_def(inputs=self.inputs,
    ...
    def build_inputs_and_outputs(self):
        if self.frame_features:
            ...
            video_id_output, top_indices_output, top_predictions_output = \
                    (tf.map_fn(fn, serialized_examples, dtype=(tf.string, tf.int32, tf.float32)))
        else:
            serialized_examples = tf.placeholder(tf.string, shape=(None,))
            video_id_output, top_indices_output, top_predictions_output = (self.build_prediction_graph(serialized_examples))
        inputs = {"example_bytes": saved_model_utils.build_tensor_info(serialized_examples)}
        outputs = {"video_id": saved_model_utils.build_tensor_info(video_id_output),
    ...
    def build_prediction_graph(self, serialized_examples):
        video_id, model_input_raw, labels_batch, num_frames = (self.reader.prepare_serialized_examples(serialized_examples))
