...
class ModelExporter(object):
    ...
    def export_model(self, model_dir, global_step_val, last_checkpoint):
        with self.graph.as_default():
            with tf.Session() as sess:
                ...
                signature = build_signature_def(
                        inputs=self.inputs,
                        outputs=self.outputs,
                        method_name=signature_constants.PREDICT_METHOD_NAME)
                signature_map = {signature_constants\
                                 .DEFAULT_SERVING_SIGNATURE_DEF_KEY: signature}
                model_builder = SavedModelBuilder(model_dir)
    ...
    def build_inputs_and_outputs(self):
        ...
        inputs = {"example_bytes": build_tensor_info(serialized_examples)}
        outputs = {"video_id": build_tensor_info(video_id_output),
                   "class_indexes": build_tensor_info(top_indices_output),
                   "predictions": build_tensor_info(top_predictions_output)}
    ...
    def build_prediction_graph(self, serialized_examples):
        ...
        with tf.name_scope("model"):
            ...
            top_predictions, top_indices = tf.nn.top_k(
                    predictions, _TOP_PREDICTIONS_IN_OUTPUT)
